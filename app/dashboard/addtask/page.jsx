...
function AddTask() {
  ...
  const handleSubmit = async (e) => {
    ...
    const modifiedCollaborators = collaborator.map((dept) => ({
      dept: dept,
      isRequested: false,
      isConflicting: hasConflicts,
    }));

    if (hasConflicts || hasCollaborators) {
      const userConfirmed = window.confirm(
        "Conflicting tasks found. Do you want to proceed anyway?"
      );
      if (!userConfirmed) {
        setLoading(false);
        return;
      ...
      setIsPending(true);
      openModal();
      await saveTaskToFirebase(true, conflicts, modifiedCollaborators);
    } else {
      setIsSubmitBlocked(false);
      setIsPending(false);
      await saveTaskToFirebase(false, [], modifiedCollaborators);
  ...
  const saveTaskToFirebase = async (isPending, conflicts, collaborators) => {
    try {
      const docRef = await addDoc(collection(db, "tasks"), {
        ...
        endDate,
        isCollab: iscollab,
        collaborator: iscollab ? collaborators : null,
        latitude,
        longitude,
        resourceAllocation,
        userEmail: userDetails.email,
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      ...
      <button
        onClick={openModal}
        className="flex items-center justify-center bg-red-500 my-4 mx-auto text-white px-4 py-2 rounded hover:bg-red-600 shadow-md transition duration-200"
      >
        See Conflicts
      </button>
      ...
      <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col">
            <label htmlFor="startDate" className="mb-1 text-gray-700">
              Start Date
            </label>
            <input
              type="date"
          ...
          <div className="flex flex-col">
            <label htmlFor="endDate" className="mb-1 text-gray-700">
              End Date
            </label>
            <input
              type="date"
        ...
        <div className="flex items-center">
          <input
            ...
            className="mr-2"
          />
          <label htmlFor="iscollab" className="text-gray-700">
            Is this task collaborative?
          </label>
        </div>

        {iscollab && (
          <div className="flex flex-col">
            <label htmlFor="collaborator" className="mb-1 text-gray-700">
